...
+ [Где используется `Comparator`?](#Где-используется-Comparator)

[к оглавлению](#Java-Collections-Framework)

## Что такое _«коллекция»_?
_Коллекция_ - это объект, структура данных, которая служит набором других объектов. Данными в коллекции могут быть числа, строки, пользовательские классы и т.п.
...
+ Также есть важные статические поля `TREEIFY_THRESHOLD = 8`, `UNTREEIFY_THRESHOLD = 6` и `MIN_TREEIFY_CAPACITY = 64`, первое из которых содержит число элементов в корзине, при достижении которого корзина будет перестроена из односвязного списка в сбалансированное дерево (при условии также, что размер массива внутри мапы не меньше чем третяя константа), построенное на `TreeNode` (вложенном в `HashMap` классе). Это позволит сократить худшее время поиска элемента с __O(N)__ до __O(log(N))__. Второе поле содержит число элементов в сбалансированном дереве, при котором оно обратно перестраивается в список. Оптимизация со сбалансированным деревом была введена в Java 8.
...
+ `TreeMap` может работать только при наличии компаратора, или, при его отсутствии, только с объектами класса, реализующего `Comparable`.
...
+ Как и `TreeMap`, может работать только при наличии компаратора, или, при его отсутствии, только с объектами класса, реализующего `Comparable`.
...
+ Замена массива на более большой происходит при 100-процентной заполнености и в зависимости от текущего размера прирост ячеек будет разным. Для маленького размера массив увеличивается в `2` раза, для более больших - в `1.5` раза.
+ Замена массива происходит быстро за счет использования `System.arrayCopy()`.
+ Не позволяет добавлять `null` элементы, а также элементы, которые не реализуют интерфейс `Comparable` (в случае, если компаратор не задан).
...
## `Iterable`
+ Дженериковый интерфейс, который позволяет объектам класса-реализации быть итерируемыми.
+ Реализация `Iterable` позволяет объектам класса-реализации участвовать в циклах `for-each` :
```java
for (String str : list) {
    doSmth(str);
} 
```
+ Имеет `3` метода, `1` без тела (был с самого начала) и `2` дефолтных (появились в Java 8) :
    + `Iterator<T> iterator()` - возвращает `Iterator` для данного объекта.
    + `default forEach(Consumer<? super T> action) -> void` - по дефолту применяет переданное действие к каждому элементу объекта с помощью `foreach` цикла.
    + `default spliterator() -> Spliterator<T>` - возвращает `Spliterator` для данного объекта. По дефолту позвращает объект `Spliterators.spliteratorUnknownSize()` со всеми характеристиками установленными в `0`.
+ Является `parent` - интерфейсом для `Collection`, поэтому все коллекции который реализуют `Collection` (то есть, кроме `Map`), итерируемы.
+ Интерфейс `Iterable` появился в Java 5.
...
## `Comparable`
+ Дженериковый интерфейс, который означает, что объекты данного класса сравнимы с объектами класса `T` (параметра в `Comparable<T>`).
+ В интерфейсе всего `1` метод : `compareTo(T o) -> int`. Возможные его возвращаемые значения и выбрасываемые исключения :
    + `-1`, если объект, на котором вызывается этот метод, _меньше_ передаваемого объекта.
    + `0`, если объект, на котором вызывается этот метод, _равен_ передаваемому объекту.
    + `1`, если объект, на котором вызывается этот метод, _больше_ передаваемого объекта.
...
## `Comparator`
+ Функциональный дженериковый интерфейс, который позволяет нам сравнивать два объекта одного класса.
+ Имеет всего один нереализованый метод `compare(T o1, T o2) -> int`. Возможные его возвращаемые значения и и выбрасываемые исключения аналогичные вызову `o1.compareTo(o2)`:
    + `-1`, если если первый объект, _меньше_ второго объекта.
    + `0`, если первый объект _равен_ второму объекту.
    + `1`, если первый объект _больше_ второго объекта.
    + `NullPointerException`, если один из передаваемых объектов `null`.
    + `ClassCastException`, если тип одного из передаваемых объектов не позволяет ему быть сравнимым с другим объектом.
+ В `Comparator` есть ряд дефолтных методов, самые интересные из которых :
    + `thenComparing(Comparator<? super T> other) -> Comparator<T>` - позволяет использовать цепочки компараторов. Это работает так, что если первый компаратор вернул результат `0`, то вызывается второй с этими же аргументами и так далее.
    + `reversed() -> Comparator<T>` - возвращает компаратор, который будет выдывать результаты, противоположные текущему компаратору.
+ В `Comparator` также есть дефолтные статические фабричные методы :
    + `naturalOrder() -> Comparator<T>` - возвращает компаратор, который сортирует `Comparable` объекты в натуральном порядке.
    + `nullsFirst(Comparator<? super T> comparator) -> Comparator<T>` - преобразовывает компаратор в такой, что будет считать `null` значения меньше чем остальные.
    + `nullsLast(Comparator<? super T> comparator) -> Comparator<T>` - преобразовывает компаратор в такой, что будет считать `null` значения больше чем остальные.
    + `2` фабричных метода `comparing(keyExtractor, keyComparator)` и `comparing(keyExtractor)`, которые основываются на функции, которая достает ключи для сравнения из объектов. Возвращаемый компаратор строится на сравнении этих ключей.
    + `3` фабричных метода для сравнения `int`, `long` и `double` значений - `comparingInt(ToIntFunction<? super T> keyExtractor) -> Comparator<T>`, `comparingLong(ToLongFunction<? super T> keyExtractor) -> Comparator<T>` и `comparingDouble(ToDoubleFunction<? super T> keyExtractor) -> Comparator<T>` соответственно.
+ Все дефолтные методы в `Comparator` были добавлены в Java 8.
+ Сам интерфейс `Comparator` был добавлен в Java 2.

[к оглавлению](#Java-Collections-Framework)

## Где используется `Comparator`?
`Comparator` используется в таких коллекциях :
+ `TreeMap` и других реализациях `SortedMap`
+ `TreeSet` и других реализациях `SortedSet`
+ `PriorityQueue`
