...
+ [Статические методы интерфейсов](#Статические-методы-интерфейсов)
    + [Пример статического метода интерфейса](#Пример-статического-метода-интерфейса)
...
+ [Streams](#Streams)
    ...
    + [Метод `Collection.stream()`](#Метод-Collectionstream)
    + [Метод `Collection.parallelStream()`](#Метод-Collectionparallelstream) 
...
+ Статические методы интерфейсов ([_перейти_](#Статические-методы-интерфейсов))
...
В `Java 8` были добавлены статические методы для интерфейсов.

+ Статические методы в интерфейсе являются частью интерфейса, мы не можем использовать их для объектов класса реализации.
...
+ Мы не можем определить статические методы для методов класса `Object`, потому что получим ошибку компиляции: _"This static method cannot hide the instance method from Object"_. То есть `Object` является базовым классом для всех классов и мы не можем использовать статический метод и еще такой метод с одинаковой сигнатурой.
...
## Пример статического метода интерфейса
```java
...
public class MyDataImpl implements MyData {
    ...
    public static void main(String[] args){
        ...
        obj.print("");                      // Статический метод проверки на null
        obj.isNull("abc");
    }
}
```
В данном примере мы создаем интерфейс со статическим методом и его реализацию, где создается экземплярный метод с таким же именем. При вызове дефолтного метода `print` метод `isNull` будет вызываться именно как статический метод интерфейса, а не метод экземпляра, написанный в реализации.
