...
+ Records - замена POJO классам, предзначены для сущностей, которые имеют состояние, но не имеют поведение - пока в _Preview_
+ Pattern matching для `instanceof` - неявное приведение типа сразу `в instanceof` - пока в _Preview_
